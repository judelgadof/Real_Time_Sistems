...
float ntc_read_temperature ( void ) {
    ...
    int raw_value = adc_read_raw_multi ( &adc_two, 1);

    // Convertir la lectura cruda a voltaje en mV
    float voltage = ( ( float ) raw_value / ( ( 1 << ADC_BITWIDTH_12 ) - 1 ) ) * ADC_VOLTAGE; 

    // Validar el rango del voltaje
    if ( voltage <= 0 || voltage >= ADC_VOLTAGE ) {
        return -1000.0; // Valor de error
    }

    // Calcular la resistencia del NTC con la fórmula del divisor de voltaje
    ...
    float temperature = ( ( 1.0 / ( ( ( log ( resistance / NTC_R25 ) ) / NTC_BETA ) + ( 1.0 / T_KELVIN ) ) ) - 273.15 );  
